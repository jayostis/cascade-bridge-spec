...
def test_reports_nothing_for_envelopes_that_conform(crate):
    assert not shape_file_messages(crate, "envelope.ttl")


def test_reports_an_envelope_whose_name_is_not_a_short_id(crate):
...
def test_reports_an_envelope_that_names_no_doc_root_element_name(crate):
    envelope = next(crate.graph.objects(crate.root, BRIDGE.envelope))
