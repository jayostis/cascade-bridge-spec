...
def test_an_entry_whose_engine_passes_its_adapter_holds(world):
    ...
    assert "holds; 1 cantTell, 3 passed, 1 untested" in said
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "failed",
            ["does not hold; 1 cantTell, 1 failed, 2 passed, 1 untested"],
        ...
        pytest.param(
            "partial",
            [
                "does not hold; 1 passed; 4 of the manifest's 5 tests have no outcome: "
                "example-0002, example-0003, example-0004, example-release-2026-01"
            ],
